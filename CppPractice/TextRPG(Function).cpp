...
bool LoadPlayer(_tagPlayer* pPlayer) {
	...
	if (pFile) {
		...
		//만약 저장할때 무기를 차고 있었다면 해당 무기정보도 같이 저장되어있음.
		//그러므로 여기서 차고 있을경우 읽어와야함.
		if (pPlayer->bEquip[EQ_WEAPON]) {
...
bool SavePlayer(_tagPlayer* pPlayer) {
	...
	if (pFile) {
		...
		//만약 저장할때 무기를 차고 있었다면 해당 무기정보도 같이 저장해야함.
		//그러므로 여기서 차고 있을경우 저장해야함.
		if (pPlayer->bEquip[EQ_WEAPON]) {
...
int main(){
	...
	case GM_LOAD:
		//플레이어의 정보를 불러옴
		//LoadPlayer의 반환값이 false일 경우 실행
		if (!LoadPlayer(&tPlayer)) {
			system("cls");
			cout << "저장된 플레이어의 정보가 존재하지 않습니다!" << endl;
			return 0;
		}
		else {
